...
function createComponent(staticConfig) {
  ...
  const component = forwardRef((propsIn, forwardedRef) => {
    ...
    const isAnimatedReactNative = hasAnimationProp && animationsConfig?.isReactNative, isReactNative = !!(staticConfig.isReactNative || isAnimatedReactNative), shouldAvoidClasses = !!(!isWeb || isAnimated || !staticConfig.acceptsClassName || propsIn.disableClassName), shouldForcePseudo = !!propsIn.forceStyle, noClassNames = shouldAvoidClasses || shouldForcePseudo, disableTheme = !1 && !willBeAnimated || isHOC;
...
` && console.error(
    ...
    willBeAnimated && presence && ResetPresence && typeof content != "string" && (content = /* @__PURE__ */ jsx(ResetPresence, { children: content })), process.env.NODE_ENV === "development" && time && time`create-element`;
    const groupState = stateRef.current.group, subGroupContext = useMemo(() => {
